...
- (void)startRequest
{
    ...
    NSDictionary *theDecodedEvent = [theJSONParser objectWithString:theEventJSON error:&theError];
...
- (void)dealloc
{
    [_eventRequest release];
    [_infoPopoverController release];
...
{
    UIViewController *theInfoViewController = [[EventsInfoViewController new] autorelease];
    ...
    if (UIUserInterfaceIdiomPad == UI_USER_INTERFACE_IDIOM())
    {
        if (nil == self.infoPopoverController)
        {
            self.infoPopoverController = [[[UIPopoverController alloc]
                        initWithContentViewController:theInfoViewController] autorelease];
        }
