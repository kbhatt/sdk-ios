    ...
    if ([prop isEqualToString:@"selected" ignoreCase:YES]) { 
        ...
        if (shouldReturnArray) {
            ...
            for (int j = 0; j < [self numberOfSections]; j++) {
                ...
                result = result = [result stringByAppendingFormat:@"["];
...
//		}
		...
		[self.dataSource origTableView:self commitEditingStyle:style forRowAtIndexPath:path];
                ...
                [event.command isEqualToString:MTCommandLongSelectIndex ignoreCase:YES] ||
        ...
        UITableViewCell *selectCell = [self cellForRowAtIndexPath:indexPath];
...
//                [self.delegate tableView:self didSelectRowAtIndexPath:indexPath];
                ...
                if ([event.command isEqualToString:MTCommandSelectIndicator ignoreCase:YES]){
                    ...
                    [UIEvent performTouchInView:cell.contentView];
            }); 
        });
        ...
        [self.delegate originalTableView:self moveRowAtIndexPath:sourceIndexPath toIndexPath:destinationIndexPath];
...
+ (NSIndexPath *) indexPathForCellTextLabel:(UITableView *)tableView withTitle:(NSString *)title {
    ...
    for (int i = 0; i < [tableView numberOfSections]; i++) {
        for (int j = 0; j < [tableView numberOfRowsInSection:i]; j++) {
            ...
            UITableViewCell *cell = [tableView.delegate tableView:tableView cellForRowAtIndexPath:currentPath];
