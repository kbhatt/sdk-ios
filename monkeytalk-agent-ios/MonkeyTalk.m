...
@synthesize commands, runTimeout, state, session, csvData, previousTime, commandSpeed, isWireRecording, recordHost, recordPort, foundComponents, currentOrientation, isAnimating, currentCommand;
...
- (void) recordEvent:(MTCommandEvent*)event {
    ...
    if ([[event source] respondsToSelector:@selector(accessibilityIdentifier)])
        accId = [[event source] accessibilityIdentifier];
    
    if (![[event source] isKindOfClass:objc_getClass("UITabBarButton")]) {
        if (accId!=nil && [accId length]>0)
            event.monkeyID = accId;
        else if (accLbl!=nil && [accLbl length]>0)
    ...
    NSString *recordString = [NSString stringWithFormat:@"\n\n< < < RECORDING > > > - source:%@\n%@ %@",[[event source] class], [event command], event.monkeyID];
...
- (void) dealloc {
    [commandSpeed release];
    [recordHost release];
    [recordPort release];
    [foundComponents release];
    [csvData release];
...
- (void) runCommandsStartingFrom:(NSInteger)start numberOfCommands:(NSInteger)count{
	...
	for (i = start; i < start + count; i++) {
		...
		} else if ([nextCommandToRun.command isEqualToString:MTCommandWaitFor]) {
            // WaitFor, Shake, Rotate may need to be on main thread
			[MTWaitForCommand execute:nextCommandToRun];
...
- (void) playbackMonkeyEvent:(MTCommandEvent*)command {
    ...
    } else if([source isKindOfClass:objc_getClass("MTFoundElement")]) {
        // Played in web view
        [source release];
    } else {
        NSString *mtComponent = [MTConvertType convertedComponentFromString:command.className isRecording:YES];
        command.lastResult = [NSString stringWithFormat:@"No %@ found with monkeyID \"%@\"", mtComponent, command.monkeyID];
    }  
}
...
- (NSString *) playAndRespond:(MTCommandEvent*)nextCommandToRun {
    ...
    if ([nextCommandToRun.command isEqualToString:MTCommandDataDrive]) {
        ...
        NSLog(@"\n\n< < <Playing> > > %@ %@ \"%@\" %@\n\n", 
              nextCommandToRun.command,
              nextCommandToRun.className != nil ? nextCommandToRun.className : @"",
              nextCommandToRun.monkeyID, 
              nextCommandToRun.args);
    ...
    if (retryTime > 0)
        commandPlaybackSpeed = MT_DEFAULT_RETRY_DELAY*1000;
    
    if ([nextCommandToRun.className isEqualToString:MTComponentDevice ignoreCase:YES]) {
    ...
    if ([nextCommandToRun lastResult]) {
        ...
        NSArray *messageKeys = [NSArray arrayWithObjects:@"screenshot", @"message", nil];
        NSArray *messageObjects = [NSArray arrayWithObjects:[MTUtils encodedScreenshot], nextCommandToRun.lastResult, nil];
        NSDictionary *messageDict = [NSDictionary dictionaryWithObjects:messageObjects forKeys:messageKeys];
...
- (void) postCommandFrom:(UIView*)sender command:(NSString*)command args:(NSArray*)args {
	...
	if (!_lastCommandRecorded && lastCommandPosted.monkeyID && lastCommandPosted.monkeyID != [sender monkeyID]) {
		[self recordEvent:lastCommandPosted];
	} else if (([lastCommandPosted.command isEqualToString:MTCommandTouchMove] && !_lastCommandRecorded) ||
	...
	if (sender) {
        if ([sender isKindOfClass:objc_getClass("UINavigationItemButtonView")])
            lastCommandPosted.monkeyID = [sender title];
        else
...
- (MTCommandEvent*)commandAt:(NSInteger)index {
	...
	return [[[MTCommandEvent alloc] initWithDict:dict] autorelease];
...
- (NSString*) monkeyIDfor:(UIView*)view {
	...
	value = [NSString stringWithFormat:@"#%d", [MTUtils ordinalForView:view]];
