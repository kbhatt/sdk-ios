...
#import "MTHTTPPNGResponse.h"
#import "NSException+MTWebDriver.h"
#import "MTerrorcodes.h"

@implementation MTWebDriverResponse
...
- (id)initWithValue:(id)theValue {
  ...
  // The sessionId is set automatically by |MTSession|.
...
- (id)initWithError:(id)error {
  NSLog(@"Creating MTWebDriverResponse with:\n%@", [error description]);
...
- (void)createResponse {
  ...
  if ([value_ isKindOfClass:[UIImage class]]) {
    response_ = [[MTHTTPPNGResponse alloc] initWithImage:value_];
  } else {
    NSDictionary *dict = [self convertToDictionary];
    response_ = [[MTHTTPJSONResponse alloc] initWithObject:(id)dict];
  }
}
