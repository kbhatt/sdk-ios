...
#import "MTHTTPResponse+Utility.h"
...
#import "MTSession+ExecuteScript.h"
#import "MTSessionRoot.h"
#import "MTStorage.h"
#import "MTTimeouts.h"
...
// The MTWebViewController has most of the actual functionality this vdir exposes.
// We'll just forward most messages there.
- (void)setResourceToViewMethodGET:(SEL)getMethod
                              POST:(SEL)postMethod
                          withName:(NSString *)name {
  [self setResource:[MTWebDriverResource resourceWithTarget:[self viewController]
                                                GETAction:getMethod
...
- (id) initWithSessionRootAndSessionId:(MTSessionRoot*)root
  ...
  [self setIndex:[MTWebDriverResource
  ...
  [self setResource:[MTStorage storageWithSessionId:sessionId_ 
                                          andType:LOCAL_STORAGE]
           withName:@"local_storage"];
  
  // HTML5 MTSession WebStorage
  [self setResource:[MTStorage storageWithSessionId:sessionId_ 
                                          andType:SESSION_STORAGE]
           withName:@"session_storage"];
  
  // HTML5 Get and Set MTGeoLocation
  [self setResourceToViewMethodGET:@selector(location)
                              POST:@selector(setLocation:)
                          withName:@"location"];
  
  // HTML5 MTDatabase MTStorage
  [self setResource:[MTDatabase databaseWithSessionId:sessionId_] 
  ...
  // |MTElementStore| handles the /element virtual directory and all of its
  ...
  // page.
  elementStore_ = [MTElementStore elementStoreForSession:self];

  [self setResource:[MTCookie cookieWithSessionId:sessionId_]
           withName:@"cookie"];
  
  [self setResource:[MTTimeouts timeoutsForSession:self]
           withName:@"timeouts"];
...
- (void)cleanSessionStatus{
  [MTWebDriverUtilities cleanCookies];
  [MTWebDriverUtilities cleanCache];
  [MTWebDriverUtilities cleanDatabases];
}
...
- (void)configureResource:(id<MTHTTPResource>)resource {
...
- (id<MTHTTPResource>)elementWithQuery:(NSString *)query {
  ...
  id<MTHTTPResource> resource = [super elementWithQuery:query];
