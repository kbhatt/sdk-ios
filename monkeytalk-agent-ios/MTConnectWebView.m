...
- (void) playCommand:(NSArray *)args
{
    ...
    dispatch_group_async(group, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        @synchronized(self) {
            ...
            [[MonkeyTalk sharedMonkey] playCommandFromJs:[MTCommandEvent command:commandString className:@"" monkeyID:monkeyString delay:delayString timeout:retryString args:commandArgs]];
        }
    });
...
- (void) playCommands:(NSArray *)args
{
    ...
    for (int i = 0; i < [commands count]; i++) {
        ...
        [array addObject:[MTCommandEvent command:command className:className monkeyID:monkeyId delay:playback timeout:timeout args:args]];
    }
