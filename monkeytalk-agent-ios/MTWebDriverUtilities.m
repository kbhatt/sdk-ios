...
@implementation MTWebDriverUtilities
...
+ (void)cleanDatabases{
  ...
  NSFileManager *mt = [NSFileManager defaultManager];
  if (!dbDir || ![dbDir isKindOfClass:[NSString class]] ||
    ![mt fileExistsAtPath:dbDir]) {
    return;
  }    
  ...
  if ([mt fileExistsAtPath:dbFileOfDatabases]) {
    BOOL succ = FALSE;
    ...
    if (!succ) {
      NSLog(@"Unable to open databse 'Databases'. Delete the database file '%@'",
          dbFileOfDatabases);
      [mt removeItemAtPath:dbFileOfDatabases error:nil];
    }
  }
  
  NSArray *subPaths = [mt subpathsOfDirectoryAtPath: dbDir error:nil];                       
  for (int i = 0; i < [subPaths count]; i++) {
    ...
    [mt removeItemAtPath:path error:nil];
  } 
}
