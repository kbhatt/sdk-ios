...
@implementation MTWebDriverPreferences
...
static MTWebDriverPreferences *singleton = nil;
+ (MTWebDriverPreferences*) sharedInstance {
  if (singleton == nil) {
    singleton = [[MTWebDriverPreferences alloc] init];
  }	
  return singleton;
}
...
//                              @"Settings.bundle"];
    NSString* rootPlist = [bundlePath stringByAppendingPathComponent:
                           @"Root.plist"];
    ...
    for (NSDictionary* item in preferences) {
      ...
      [defaultPrefs setObject:MT_PLAYBACK_PORT
                       forKey:PREF_SERVER_MODE_PORT_NUMBER];
...
- (id)init {
  [MTWebDriverPreferences initPreferences];
  ...
  if ([mode_ isEqualToString:@"Client"]) {
    ...
    [MTWebDriverPreferences validateConnectorAddr:connectorAddr_];
    ...
    [MTWebDriverPreferences validateRequesterId:requesterId_];
