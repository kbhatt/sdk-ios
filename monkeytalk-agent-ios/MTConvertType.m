...
+ (NSDictionary *) recordDictionary {
    ...
    // Add components value:iOS key:wire(new)
    [mutableComponents setValue:MTComponentView forKey:@"UIView"];
...
+ (NSString *) convertedComponentFromString:(NSString *)originalComponent 
    ...
    if (isRecording) {
        if (![[self recordDictionary] objectForKey:originalComponent]) {
        ...
        return [[self recordDictionary] objectForKey:originalComponent];
    }
